...
public:
    BTNode *_parent;
    Record<T> *_records[ORDER]{};
    int _count;
    BTNode *_children[ORDER + 1]{};
    int _childs;
    
    BTNode(Record<T> *record, BTNode *parent);
...
template<class T>
BTNode<T>::BTNode(Record<T> *record, BTNode *parent){
    _parent = parent;
    _records[0] = record;
    _count = record ? 1 : 0;
    _childs = 0;
...
class BTree{
    using BNode = BTNode<T> *;
    BNode _root;
    static const int LEAST_COUNT_OF_ROOT;
    static const int LEAST_COUNT_OF_INTERNAL;
...
template<class T>
const int BTree<T>::LEAST_COUNT_OF_ROOT = 1;

template<class T>
const int BTree<T>::LEAST_COUNT_OF_INTERNAL = ceil(ORDER * 0.5) - 1;

#pragma mark - Public
template<class T>
void BTree<T>::add(T v){
    if (!_root){
        _root = new BTNode<T>(new Record(0, v), NULL);
        return;
    }
...
void BTree<T>::overflow(BNode n){
    BNode right = new BTNode<T>(NULL, NULL);
    Record<T> *rd = split(n, right);
    if (n == _root){
        rd->_key = 0;
        _root = new BTNode<T>(rd, NULL);
