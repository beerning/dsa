...
class HuffmanTree{
    ...
    void insert(Array &array, Node n);
...
void HuffmanTree::insert(Array &array, Node n){
    long high = (long)array.size() - 1;
    long low = 0;
    long mid = 0;
    int v = n->_weight;
    int x = 0;
    while (high >= low) {
        ...
        x = array[mid]->_weight;
        if (v > x) low = mid + 1;
        else if (v < x) high = mid - 1;
        else break;
    }
    array.emplace(array.begin() + low, n);
}
